...
INTERFACE
 ...
 SUBROUTINE CALCBIL(c,Ham,dc,expnQ)
  USE FNVAR
  IMPLICIT NONE
  DOUBLE COMPLEX, INTENT(in) :: c(0:n),Ham(0:n,0:n)
...
DOUBLE PRECISION :: acc,r0,v0,p0,a0,b0,rtemp,QC1,QC2
DOUBLE COMPLEX :: w,w1
LOGICAL :: opt,quantum
!
...
!
DOUBLE PRECISION :: DPr,DDPr,DPtemp,xx, psave(1:4)
...
w = w - DCMPLX(2.0d0*(w*w)/mass + 0.5d0*DDPr)*(dt/2.0d0)
DDPr = DPoten(2) + QC2
w = w - DCMPLX(2.0d0*(w*w)/mass + 0.5d0*DDPr)*(dt/2.0d0)
...
!!! Calculation of the effective potential (in two 1/2 time steps)
...
DO pp = 1,2
 IF (pp.eq.1) THEN
  r = r0; v = v0; a = a0; b = b0
 ELSE
  r = psave(1); v = psave(2); a = psave(3); b = psave(4);
 ENDIF
 ...
 rtemp = r
 DO i=0,m-1
  r = SQRT(hbar/(2.0d0*a))*zm(i) + r
  DPtemp = DPoten(0)
  r = rtemp
  EPoten(i)=DPtemp-DPoten(0)-(DPoten(1)+QC1)*(SQRT(hbar/(2.0d0*a))*zm(i)) &
   & - 0.5d0*(DPoten(2)+QC2)*(SQRT(hbar/(2.0d0*a))*zm(i))**2

 write(*,*) zm(i)
 write(*,*) DPtemp,DPoten(0),DPoten(1),DPoten(2)
 write(*,*) DPtemp-DPoten(0)
 write(*,*) '-',(DPoten(1)+QC1)*(SQRT(hbar/(2.0d0*a))*zm(i))
 write(*,*) '-',0.5d0*(DPoten(2)+QC2)*(SQRT(hbar/(2.0d0*a))*zm(i))**2
 write(*,*) EPoten(i)
 read(*,*)
 ENDDO
...
!!! Propagation of coefficients
 DO i=0,n
  c(i) = c(i)+((0.0d0,-1.0d0)/hbar)*(Wa(i) + c(i)*(p**2/(2.0d0*mass) &
   & + (2.0d0*DBLE(i)+1.0d0)*hbar*a/mass))*(dt/2.0d0)
 ENDDO
